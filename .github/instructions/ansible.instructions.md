...
- Give every play, block, and task a concise but descriptive `name` following the format: `<Parent Role Name> | <$(basename filename.yml)> | <task name>`
  - This format provides context about which file the task is defined in, which role is currently running the task, and then the task itself.
  - Start the `<task name>` with an action verb that indicates the operation being performed, such as "Install," "Configure," or "Copy"
  - Use Title Case for task names to improve readability
...
- When using other tools with Ansible (e.g., Terraform), store secrets in a third-party secrets management tool (e.g., Hashicorp Vault, AWS Secrets Manager, 1Password, etc.)
